...
DEFAULT_GRAPH_FOLDER_NAME = "hetero_graphs"
PATIENCE = 10
NUM_EPOCHS = 50
DEFAULT_TRIALS = 10
DEFAULT_RUNS = 10
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a GNN on regular heterogeneous prefix graphs."
    )
    parser.add_argument(
        "dataset",
        nargs="?",
        default=os.environ.get("THESIS_DATASET", DEFAULT_DATASET),
        help="Dataset folder under data/datasets/hetero_graphs.",
    )
    parser.add_argument(
        "--prediction-task",
    ...
    parser.add_argument(
        "--graph-dir",
        default=os.environ.get("THESIS_HETERO_GRAPHS_DIR"),
        help=(
            "Directory that contains one folder per dataset with train/validation/test "
            "heterogeneous graph .pt files. Supports local paths and gs:// URLs."
        ),
    )
    parser.add_argument(
        "--epochs",
...
def append_runtime_report(
    ...
    path = os.path.join(results_dir, "runtime_report_hetero.csv")
...
def load_hetero_graph_splits(graph_dir: str, dataset: str) -> tuple[list[HeteroData], list[HeteroData], list[HeteroData], list[dict]]:
    dataset_import_summaries = []
    split_files = [
        "train_set_hetero.pt",
        "validation_set_hetero.pt",
        "test_set_hetero.pt",
    ]
    ...
    return (
        loaded["train_set_hetero.pt"],
        loaded["validation_set_hetero.pt"],
        loaded["test_set_hetero.pt"],
        dataset_import_summaries,
    )
...
def validate_prediction_labels(graphs: list[HeteroData], prediction_task: str) -> None:
    ...
    if missing:
        raise ValueError(
            ...
            "but at least one graph does not contain them. Regenerate the regular "
            "heterogeneous graphs with those targets or use --prediction-task next_activity."
        )
...
def test_multi(
    ...
    results_table.to_csv(os.path.join(save_path, "results_hetero.csv"), index=False)
    pd.DataFrame({"mean": means, "std": stds}).to_csv(
        os.path.join(save_path, "mean_and_stds_hetero.csv")
    )
    print(pd.DataFrame({"mean": means, "std": stds}))
    print(f"Saved final regular hetero evaluation results to: {save_path}")
    return results
...
def main() -> None:
    ...
    results_dir = os.path.join(results_root_dir, args.dataset, "hetero")
    ...
    x_train, x_valid, x_test, import_summaries = load_hetero_graph_splits(
        graph_dir,
        args.dataset,
    )
    pd.DataFrame(import_summaries).to_csv(
        os.path.join(results_dir, "dataset_import_summary_hetero.csv"),
        index=False,
    )
    ...
    search_results_path = os.path.join(results_dir, "hyp_params_search_hetero.csv")
    ...
    if args.skip_search:
        ...
        print(f"Saved regular hetero hyperparameter search results to: {search_results_path}")

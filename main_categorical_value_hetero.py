...
DEFAULT_GRAPH_FOLDER_NAME = "categorical_value_hetero_graphs"
PATIENCE = 10
NUM_EPOCHS = 50
DEFAULT_TRIALS = 10
DEFAULT_RUNS = 10
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a GNN on categorical-value heterogeneous prefix graphs."
    )
    parser.add_argument(
        "dataset",
        nargs="?",
        default=os.environ.get("THESIS_DATASET", DEFAULT_DATASET),
        help="Dataset folder under data/datasets/categorical_value_hetero_graphs.",
    )
    parser.add_argument(
        "--prediction-task",
    ...
    parser.add_argument(
        "--graph-dir",
        default=os.environ.get("THESIS_CATEGORICAL_VALUE_HETERO_GRAPHS_DIR"),
        help=(
            "Directory that contains one folder per dataset with train/validation/test "
            "categorical-value heterogeneous graph .pt files. Supports local paths and gs:// URLs."
        ),
    )
    parser.add_argument(
        "--epochs",
...
def append_runtime_report(
    ...
    path = os.path.join(results_dir, "runtime_report_categorical_value_hetero.csv")
...
def load_dataset_file(graph_dir: str, dataset: str, file_name: str) -> tuple[list[HeteroData], dict]:
    path = join_graph_path(graph_dir, dataset, file_name)
...
def load_categorical_value_graph_splits(graph_dir: str, dataset: str) -> tuple[list[HeteroData], list[HeteroData], list[HeteroData], list[dict]]:
    dataset_import_summaries = []
    split_files = [
        "train_set_categorical_value_hetero.pt",
        "validation_set_categorical_value_hetero.pt",
        "test_set_categorical_value_hetero.pt",
    ]
    ...
    return (
        loaded["train_set_categorical_value_hetero.pt"],
        loaded["validation_set_categorical_value_hetero.pt"],
        loaded["test_set_categorical_value_hetero.pt"],
        dataset_import_summaries,
    )
...
def resolve_prediction_task(requested_task: str, sample_graph: HeteroData) -> str:
...
def validate_prediction_labels(graphs: list[HeteroData], prediction_task: str) -> None:
    ...
    if missing:
        raise ValueError(
            ...
            "but at least one graph does not contain them. Regenerate the "
            "categorical-value heterogeneous graphs with those targets or use "
...
def infer_target_node_type(graphs: list[HeteroData]) -> str:
    for graph in graphs:
        if hasattr(graph, "target_node_type"):
            return str(graph.target_node_type)
    node_types, _ = collect_metadata(graphs)
    if "concept:name" in node_types:
        return "concept:name"
    if "Activity" in node_types:
        return "Activity"
    return node_types[0]
...
def test_multi(
    ...
    results_table.to_csv(
        os.path.join(save_path, "results_categorical_value_hetero.csv"),
        index=False,
    )
    pd.DataFrame({"mean": means, "std": stds}).to_csv(
        os.path.join(save_path, "mean_and_stds_categorical_value_hetero.csv")
    )
    print(pd.DataFrame({"mean": means, "std": stds}))
    print(f"Saved final categorical-value hetero evaluation results to: {save_path}")
    return results
...
def main() -> None:
    ...
    results_dir = os.path.join(results_root_dir, args.dataset, "categorical_value_hetero")
    ...
    x_train, x_valid, x_test, import_summaries = load_categorical_value_graph_splits(
        graph_dir,
        args.dataset,
    )
    pd.DataFrame(import_summaries).to_csv(
        os.path.join(results_dir, "dataset_import_summary_categorical_value_hetero.csv"),
        index=False,
    )

    node_types, edge_types = collect_metadata(x_train + x_valid + x_test)
    target_node_type = infer_target_node_type(x_train)
    ...
    search_results_path = os.path.join(
        results_dir,
        "hyp_params_search_categorical_value_hetero.csv",
    )
    ...
    def train_evaluate(config: dict) -> dict:
        ...
        if valid_loss is None:
            raise ValueError("train_evaluate did not receive 'valid_loss' from train_hgnn.")
    ...
    if args.skip_search:
        ...
        print(f"Saved categorical-value hetero hyperparameter search results to: {search_results_path}")
        print_runtime_report("hyperparameter_search", search_elapsed_seconds, machine_report)
    ...
    print_runtime_report("final_training_runs", training_elapsed_seconds, machine_report)
    append_runtime_report(
        results_dir,
        args.dataset,
        prediction_task,

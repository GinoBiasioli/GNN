...
OUTPUT_FOLDER_NAME = "hetero_graphs"
...
def encode_feature_node_values(
    trace: pd.DataFrame,
    col: str,
    ...
    if col in categorical_cols:
        values = normalize_categorical_series(trace[col]).values.reshape(-1, 1)
        encoded = encoders[col].transform(values).astype(np.float32)
        return torch.tensor(encoded, dtype=torch.float32)

    if col in numeric_cols:
        values = (
            pd.to_numeric(trace[col], errors="coerce")
            .fillna(0.0)
    ...
    raise ValueError(f"Column '{col}' is neither categorical nor numerical.")


def build_chain_edges(prefix_len: int) -> torch.Tensor:
    if prefix_len <= 1:
...
def build_position_edges(prefix_len: int) -> torch.Tensor:
    if prefix_len <= 0:
        return torch.empty((2, 0), dtype=torch.long)
    idx = list(range(prefix_len))
...
def build_prefix_hetero_graphs_from_trace(
    trace: pd.DataFrame,
    dataset_name: str,
    config: dict[str, Any],
    encoders: dict[str, OneHotEncoder],
    ...
    node_cols = list(dict.fromkeys(categorical_cols + numeric_cols))

    if activity_col not in node_cols:
        raise ValueError(
            f"Activity column '{activity_col}' must be included as a heterogeneous node type."
        )
    ...
    for prefix_len in range(min_prefix_length, max_prefix_len + 1):
        ...
        for col in node_cols:
            graph[col].x = encode_feature_node_values(
                prefix_trace,
                col,
                encoders,
            ...
            graph[col, "follows", col].edge_index = build_chain_edges(prefix_len)

        position_edges = build_position_edges(prefix_len)
        for col in node_cols:
            if col == activity_col:
                continue
            graph[activity_col, f"has_{col}", col].edge_index = position_edges
        ...
        graph.target_node_type = activity_col
        graph.graph_type = "heterogeneous"
...
def build_split_hetero_graphs(
    ...
    print(f"\nPreparing {split_name} regular heterogeneous dataset...")
    ...
    for case_id in tqdm(case_ids):
        ...
        if len(trace) > int(config.get("min_prefix_length", 2)):
            split_graphs.extend(
                build_prefix_hetero_graphs_from_trace(
                    trace,
                    dataset_name,
                    config,
                    encoders,
    ...
    print(f"{split_name} regular heterogeneous graphs created: {len(split_graphs)}")
    return split_graphs
...
def save_hetero_metadata(
    ...
    node_types = list(dict.fromkeys(config.get("categorical", []) + config.get("numerical", [])))
    activity_col = config["activity_col"]
    edge_types = [[node_type, "follows", node_type] for node_type in node_types]
    edge_types.extend(
        [[activity_col, f"has_{node_type}", node_type] for node_type in node_types if node_type != activity_col]
    )
    ...
    with open(os.path.join(save_dir, "hetero_metadata.json"), "w") as file:
        json.dump(metadata, file, indent=4)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build regular heterogeneous prefix graphs from processed event logs."
    )
    parser.add_argument(
        "dataset",
        nargs="?",
...
def main() -> None:
    ...
    output_files = [
        os.path.join(save_dir, "train_set_hetero.pt"),
        os.path.join(save_dir, "validation_set_hetero.pt"),
        os.path.join(save_dir, "test_set_hetero.pt"),
    ]
    if args.skip_existing and all(os.path.exists(path) for path in output_files):
        print("All regular heterogeneous graph files already exist. Skipping creation.")
        return

    print(root_path, processed_dir, graph_dir, sep="\n")
    print(f"Selected dataset: {dataset_name}")
    print(f"Regular heterogeneous graph configuration: {config_path}")
    ...
    train_graphs = build_split_hetero_graphs(
        splits["train"],
        "training",
        dataset_name,
        config,
    ...
    validation_graphs = build_split_hetero_graphs(
        splits["validation"],
        "validation",
        dataset_name,
        config,
    ...
    test_graphs = build_split_hetero_graphs(
        splits["test"],
        "test",
        dataset_name,
        config,
    ...
    print("\nSaved regular heterogeneous graph files:")
    for path in output_files:
        print(path)
    print(os.path.join(save_dir, "hetero_metadata.json"))

    if train_graphs:
        graph = train_graphs[0]
        print("\nExample regular heterogeneous graph:")
        print(graph)
